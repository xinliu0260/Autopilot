...
rt_bool_t mavlink_msg_heartbeat_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(fms_output), &fms_out) == RT_EOK) {
...
rt_bool_t mavlink_msg_sys_status_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(ins_output), &ins_out) != RT_EOK) {
        return false;
    }

    // if (mcn_copy_from_hub(MCN_HUB(bat_status), &bat_status) != RT_EOK) {
    //     return false;
    // }
...
rt_bool_t mavlink_msg_system_time_pack_func(mavlink_message_t* msg_t)
{
    ...
    system_time.time_unix_usec = autopilot_system_time_us();
    system_time.time_boot_ms   = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_attitude_quaternion_pack_func(mavlink_message_t* msg_t)
{
    ...
    att_quat.time_boot_ms = autopilot_system_time_us();
...
rt_bool_t mavlink_msg_local_position_ned_pack_func(mavlink_message_t* msg_t)
{
    ...
    local_position_ned.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_global_position_int_pack_func(mavlink_message_t* msg_t)
{
    ...
    global_position_int.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_altitude_pack_func(mavlink_message_t* msg_t)
{
    ...
    altitude.time_usec          = autopilot_system_time_us();
    altitude.altitude_monotonic = 0.0f;
    altitude.altitude_amsl      = ins_out.alt;
    altitude.altitude_local     = ins_out.h_R;
    altitude.altitude_relative  = fms_res == RT_EOK ? ins_out.h_R - fms_out.home[2] : 0.0f;
...
rt_bool_t mavlink_msg_gps_raw_int_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(sensor_gps), &gps_report) != RT_EOK) {
        return false;
    }

    gps_raw_int.time_usec          = autopilot_system_time_us();
...
rt_bool_t mavlink_msg_rc_channels_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(hub, &rc_channels) != RT_EOK) {
        return false;
    }

    mavlink_rc_channels.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_highres_imu_pack_func(mavlink_message_t* msg_t)
{
    ...
    rt_err_t             imu_res, mag_res, baro_res, airspeed_res;
    ...
    highres_imu.time_usec     = autopilot_system_time_us();
...
rt_bool_t mavlink_msg_distance_sensor_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(sensor_rangefinder), &rf_data) != RT_EOK) {
        return false;
    }

    distance_sensor.time_boot_ms     = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_gps_global_origin_pack_func(mavlink_message_t* msg_t)
{
    ...
    gps_global_origin.time_usec = autopilot_system_time_us();
...
rt_bool_t mavlink_msg_home_position_pack_func(mavlink_message_t* msg_t)
{
    ...
    FMS_Out_Bus             fms_out       = { 0 };
    rt_err_t               ins_res, fms_res;

    ins_res = mcn_copy_from_hub(MCN_HUB(ins_output), &ins_out);
    fms_res = mcn_copy_from_hub(MCN_HUB(fms_output), &fms_out);

    if (ins_res != RT_EOK && fms_res != RT_EOK) {
        return false;
    }

    home_position.time_usec = autopilot_system_time_us();
...
rt_bool_t mavlink_msg_attitude_target_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(auto_cmd), &auto_cmd) != RT_EOK) {
        return false;
    }

    att_target.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_scaled_imu_pack_func(mavlink_message_t* msg_t)
{
    ...
    imu_data_t           imu_data   = { 0 };
    mag_data_t           mag_data   = { 0 };
    rt_err_t            imu_res, mag_res;

    imu_res = mcn_copy_from_hub(MCN_HUB(sensor_imu0), &imu_data);
    mag_res = mcn_copy_from_hub(MCN_HUB(sensor_mag0), &mag_data);

    if (imu_res != RT_EOK && mag_res != RT_EOK) {
        return false;
    }

    scaled_imu.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_position_target_local_pack_func(mavlink_message_t* msg_t)
{
    ...
    pos_target_local.time_boot_ms = rt_tick_get_millisecond();
...
rt_bool_t mavlink_msg_hil_state_pack_func(mavlink_message_t* msg_t)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(plant_states), &plant_states) != RT_EOK) {
        return false;
    }

    hil_state.time_usec  = autopilot_system_time_us();
...
static rt_err_t task_mavgcs_init(void)
{
    /* init mavproxy */
    RT_TRY(mavproxy_init());
...
static rt_err_t task_mavobc_init(void)
{
...
rt_thread_t mav_gcs = RT_NULL;
rt_thread_t mav_obc = RT_NULL;
int task_mav_gcs_init(void){
    RT_CHECK(task_mavgcs_init());
    rt_thread_t mav_gcs = rt_thread_create("mavgcs", 
                            task_mavgcs_entry, 
                            RT_NULL, 
                            8192, 
                            MAVGCS_THREAD_PRIORITY,
                            20);
    if(mav_gcs != RT_NULL){
        rt_thread_startup(mav_gcs);
        return 0;
    }
    return -1;
}

int task_mav_obc_init(void){
    RT_CHECK(task_mavobc_init());
    rt_thread_t mav_obc = rt_thread_create("mav_obc", 
        task_mavobc_entry, 
        RT_NULL, 
        4096, 
        MAVOBC_THREAD_PRIORITY, 
        20);
    if(mav_obc!=RT_NULL){
        rt_thread_startup(mav_obc);
        return 0;
    }
    return -1;
}
