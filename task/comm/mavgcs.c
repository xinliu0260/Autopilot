...
static rt_err_t handle_mavlink_message(mavlink_message_t* msg, mavlink_system_t this_system)
{
    switch (msg->msgid) {
    ...
    case MAVLINK_MSG_ID_PARAM_SET: {
        if (this_system.sysid == mavlink_msg_param_set_get_target_system(msg)) {
            ...
            if (mavlink_param_set(param_name, param_set.param_value, param_set.param_type) != RT_EOK) {
    ...
    case MAVLINK_MSG_ID_COMMAND_INT:
        if (this_system.sysid == mavlink_msg_command_long_get_target_system(msg)) {
            ...
            if (mcn_copy_from_hub(MCN_HUB(mission_data), &mission_data) == RT_EOK) {
                /* check if there is no ongoing mission */
                if (mission_data.valid_items == 0) {
                    if (command.command == MAV_CMD_DO_REPOSITION) {
                        mission_data.timestamp = rt_tick_get_millisecond();
                        ...
                        if (mcn_publish(MCN_HUB(mission_data), &mission_data) == RT_EOK) {
    ...
    case MAVLINK_MSG_ID_FILE_TRANSFER_PROTOCOL: {
        ...
        //     if (ftp_process_request(ftp_protocol_t.payload, msg->sysid, msg->compid) == RT_EOK) {
	...
	case MAVLINK_MSG_ID_LOG_REQUEST_LIST: {
		if(this_system.sysid == mavlink_msg_log_request_list_get_target_system(msg)) {
			...
			mavlink_msg_log_entry_encode(this_system.sysid,this_system.compid, msg,&entry);
			mavproxy_send_immediate_msg(MAVPROXY_GCS_CHAN,msg,true);
    ...
    //     environment_info.timestamp = rt_tick_get_millisecond();
...
rt_err_t mavgcs_init(void)
{
    ...
    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_HEARTBEAT, 1, mavlink_msg_heartbeat_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_SYS_STATUS, 1, mavlink_msg_sys_status_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_SYSTEM_TIME, 1, mavlink_msg_system_time_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_EXTENDED_SYS_STATE, 1, mavlink_msg_extended_sys_state_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_ATTITUDE, 50, mavlink_msg_attitude_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_LOCAL_POSITION_NED, 50, mavlink_msg_local_position_ned_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_GLOBAL_POSITION_INT, 10, mavlink_msg_global_position_int_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_VFR_HUD, 5, mavlink_msg_vfr_hud_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_ALTITUDE, 10, mavlink_msg_altitude_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_GPS_RAW_INT, 10, mavlink_msg_gps_raw_int_pack_func, true));

    RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_RC_CHANNELS, 50, mavlink_msg_rc_channels_pack_func, true));

#ifdef SIH_SIM_MODE
    // RT_TRY(mavproxy_register_period_msg(MAVPROXY_GCS_CHAN, MAVLINK_MSG_ID_HIL_STATE, 50, mavlink_msg_hil_state_pack_func, true));
#endif

    /* register gcs mavlink handler */
    RT_TRY(mavproxy_monitor_register_handler(MAVPROXY_GCS_CHAN, handle_mavlink_message));
