...
rt_err_t task_status_init(void)
{
    fms_out_nod = mcn_subscribe(MCN_HUB(fms_output), NULL,RT_NULL);
    RT_ASSERT(fms_out_nod != NULL);

    ins_out_nod = mcn_subscribe(MCN_HUB(ins_output), NULL,RT_NULL);
    RT_ASSERT(ins_out_nod != NULL);

    pilot_cmd_nod = mcn_subscribe(MCN_HUB(pilot_cmd), NULL,RT_NULL);
    RT_ASSERT(pilot_cmd_nod != NULL);

    mission_data_nod = mcn_subscribe(MCN_HUB(mission_data), NULL,RT_NULL);
...
int task_status_create(void){
    RT_CHECK(task_status_init());
    rt_thread_t tid = rt_thread_create("status", 
                        task_status_entry, 
                        RT_NULL, 
                        4096, 
