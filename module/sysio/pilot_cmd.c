...
uint8_t stickMapping[4];
int16_t rcChannel[16];
int16_t rcTrimChannel[16];
...
#ifdef RT_OUTPUT_PILOT_CMD
static void mavlink_send_pilot_cmd(void)
{    
	DEFINE_TIMETAG(pilot_cmd_intv, 100);

    if (check_timetag(TIMETAG(pilot_cmd_intv)) == 0) {
        return ;
    }
...
rt_err_t pilot_cmd_collect(void)
{
    uint32_t time_now = rt_tick_get_millisecond();
    uint8_t update = 0;
    ...
    RT_TRY(rt_device_control(rcDev, RC_CMD_CHECK_UPDATE, &update));
...
rt_err_t pilot_cmd_set_device(const char* dev_name)
{
    ...
    /* open new device */
    RT_TRY(rt_device_open(new_dev, RT_DEVICE_OFLAG_RDWR));
...
rt_err_t pilot_cmd_set_chan_num(uint8_t chan_num)
{
...
rt_err_t pilot_cmd_init(void)
{
    /* advertise pilot command topic */
    RT_CHECK(mcn_advertise(MCN_HUB(pilot_cmd), echo_pilot_cmd));
    RT_CHECK(mcn_advertise(MCN_HUB(rc_channels), echo_rc_channels));
    RT_CHECK(mcn_advertise(MCN_HUB(rc_trim_channels), echo_rc_channels));
