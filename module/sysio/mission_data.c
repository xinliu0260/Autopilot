...
static int mission_data_echo(void* param)
{
    ...
    console_printf("valid items: %d\n", mission_data.valid_items);

    console_printf("seq: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.seq[i]);
    }
    console_printf("]\n");

    console_printf("command: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.command[i]);
    }
    console_printf("]\n");

    console_printf("frame: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.frame[i]);
    }
    console_printf("]\n");

    console_printf("current: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.current[i]);
    }
    console_printf("]\n");

    console_printf("autocontinue: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.autocontinue[i]);
    }
    console_printf("]\n");

    console_printf("mission_type: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.mission_type[i]);
    }
    console_printf("]\n");
    ...
    console_printf("param1: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%.3f,", mission_data.param1[i]);
    }
    console_printf("]\n");

    console_printf("param2: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%.3f,", mission_data.param2[i]);
    }
    console_printf("]\n");

    console_printf("param3: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%.3f,", mission_data.param3[i]);
    }
    console_printf("]\n");

    console_printf("param4: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%.3f,", mission_data.param4[i]);
    }
    console_printf("]\n");

    console_printf("x: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.x[i]);
    }
    console_printf("]\n");

    console_printf("y: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%d,", mission_data.y[i]);
    }
    console_printf("]\n");

    console_printf("z: [");
    for (i = 0; i < mission_data.valid_items; i++) {
        console_printf("%.3f,", mission_data.z[i]);
    }
    console_printf("]\n");
...
rt_err_t mission_reset(void)
{
    /* reset current sequence number */
    cur_seq = 0;

    data_bus.timestamp = rt_tick_get_millisecond();
    ...
    RT_TRY(mcn_publish(MCN_HUB(mission_data), &data_bus));

    return RT_EOK;
}

rt_err_t clear_mission_data(const char* path)
{
    if (unlink(path) != 0) {
        console_printf("fail to remove mission file %s\n", path);
        return RT_ERROR;
    }
...
rt_err_t load_mission_data(const char* path)
{
    FILE* fp;
    uint16_t cnt = 0;
    ...
    if (fp != NULL) {
        ...
        while (!feof(fp) && cnt < mission_count) {
            ...
            */
            fscanf(fp, "%hu %hhu %hhu %hu %f %f %f %f %d %d %f %hhu", &mdp->seq, &mdp->current, &mdp->frame, &mdp->command, &mdp->param1, &mdp->param2, &mdp->param3, &mdp->param4, &mdp->x, &mdp->y, &mdp->z, &mdp->autocontinue);
...
rt_err_t mission_data_collect(void)
{
    ...
    if (mcn_poll(fms_out_nod)) {
        ...
        RT_TRY(mcn_copy(MCN_HUB(fms_output), fms_out_nod, &fms_out));
...
rt_err_t mission_data_init(void)
{
    RT_TRY(mcn_advertise(MCN_HUB(mission_data), mission_data_echo));
    ...
    RT_TRY(load_mission_data(MISSION_FILE));
    RT_TRY(mission_reset());
