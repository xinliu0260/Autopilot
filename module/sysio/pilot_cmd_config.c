...
static rt_err_t pilot_cmd_parse_command(const toml_array_t* curarr)
{
    int i, j;
    toml_table_t* tab;
    toml_array_t* arr;
...
static rt_err_t pilot_cmd_parse_mode(const toml_array_t* curarr)
{
    int i, j;
    toml_table_t* tab;
    toml_array_t* arr;
...
static rt_err_t pilot_cmd_parse_device(const toml_table_t* curtab)
{
    int i;
    const char* key;
...
rt_err_t pilot_cmd_toml_config(toml_table_t* table)
{
    int i, j;
    const char* key;
    toml_table_t* tab;
    ...
    for (i = 0; 0 != (key = toml_key_in(table, i)); i++) {
        if (MATCH(key, "stick-channel")) {
            ...
            if ((tab = toml_table_in(table, key)) != 0) {
                if (pilot_cmd_parse_device(tab) != RT_EOK) {
            ...
            if (toml_array_table_in(table, key, &arr) == 0) {
                if (pilot_cmd_parse_mode(arr) != RT_EOK) {
            ...
            if (toml_array_table_in(table, key, &arr) == 0) {
                if (pilot_cmd_parse_command(arr) != RT_EOK) {
    ...
    RT_TRY(pilot_cmd_set_device(rcDevInfo.name));
