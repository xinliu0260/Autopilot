...
#define MAV_CONSOLE_RX_BUFFER_SIZE 256
#define MAV_CONSOLE_TX_BUFFER_SIZE 1024

static struct rt_device* device;
struct rt_device mavlink_console_dev;
static struct rt_ringbuffer* _mav_console_rx_rb = RT_NULL;
static struct rt_ringbuffer* _mav_console_tx_rb = RT_NULL;
rt_align(32) uint8_t _mav_console_rx_buffer[MAV_CONSOLE_RX_BUFFER_SIZE];
rt_align(32) uint8_t _mav_console_tx_buffer[MAV_CONSOLE_TX_BUFFER_SIZE];
...
void mavlink_console_handle_timeout(void)
{
    ...
    while (rt_ringbuffer_data_len(_mav_console_tx_rb) >= 70) {

        serial_control.count = 70;
        rt_ringbuffer_get(_mav_console_tx_rb, serial_control.data, serial_control.count);

        send_serial_control_msg(&serial_control);
    }

    uint32_t len = rt_ringbuffer_data_len(_mav_console_tx_rb);
...
static rt_ssize_t _read(struct rt_device* dev,
                       rt_off_t pos,
                       void* buffer,
                       rt_size_t size)
{
    ...
    len = rt_ringbuffer_data_len(_mav_console_rx_rb);

    if (!len)
        return 0;

    size = size > len ? len : size;
    return (rt_ssize_t)rt_ringbuffer_get(_mav_console_rx_rb, buffer, size);
}

static rt_ssize_t _write(struct rt_device* dev,
                        rt_off_t pos,
                        const void* buffer,
                        rt_size_t size)
{
    ...
    while (left) {
        ...
        data_in_buffer = rt_ringbuffer_data_len(_mav_console_tx_rb);
        ...
        rt_ringbuffer_put(_mav_console_tx_rb, buffer + sent_cnt, len);
    ...
    if (rt_ringbuffer_data_len(_mav_console_tx_rb)) {
...
void mavlink_console_process_rx_msg(const mavlink_serial_control_t* serial_control)
{
    ...
    for (uint8_t i = 0; i < serial_control->count; i++) {
        if (!rt_ringbuffer_putchar(_mav_console_rx_rb, serial_control->data[i]))
            break;
    }
...
rt_err_t mavlink_console_init()
{
    ...
    if (_mav_console_rx_rb == RT_NULL) {
		_mav_console_rx_rb = (struct rt_ringbuffer *)rt_malloc(sizeof(struct rt_ringbuffer));
        if (_mav_console_rx_rb == RT_NULL) {
            console_printf("mav console ringbuffer create fail\n");
            return RT_ERROR;
        }
		rt_ringbuffer_init(_mav_console_rx_rb,_mav_console_rx_buffer,MAV_CONSOLE_RX_BUFFER_SIZE);
    }

    if (_mav_console_tx_rb == RT_NULL) {
		_mav_console_tx_rb = (struct rt_ringbuffer *)rt_malloc(sizeof(struct rt_ringbuffer));
        if (_mav_console_tx_rb == RT_NULL) {
            console_printf("mav console ringbuffer create fail\n");
            return RT_ERROR;
        }
		rt_ringbuffer_init(_mav_console_tx_rb,_mav_console_tx_buffer,MAV_CONSOLE_TX_BUFFER_SIZE);
    }

    /* register timer event */
    rt_timer_init(&timer_mav_console, "mav_console", mav_console_timeout, RT_NULL, 50, RT_TIMER_FLAG_ONE_SHOT );
