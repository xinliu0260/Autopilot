...
#ifdef RT_RECORD_CALIBRATION_DATA
static int __cfd = -1;
...
static void gyr_mavlink_calibration(void)
{
    ...
    if (mcn_copy_from_hub(MCN_HUB(sensor_imu0_0), &imu_report) != RT_EOK) {
        return;
    }
...
static void mag_calibration(void)
{
    ...
    case 2: {
        ...
        if (rotat) {
            ...
            if (mcn_copy_from_hub(MCN_HUB(sensor_mag0_0), &mag_report) != RT_EOK) {
                return;
            }
...
static void level_calibration_init(void)
{
    ...
    mavcmd_calib_level.start_time = rt_tick_get_millisecond();
