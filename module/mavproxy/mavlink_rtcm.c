    ...
    for (uint8_t i = 0; i < rtcm_dev_num; i++) {
        if (rtcm_dev[i] != NULL) {
            if (rt_device_write(rtcm_dev[i], 0, data, len) > 0) {
                ...
                rt_completion_wait(&tx_cplt[i], 100);
            }
        }
    }
}
...
void handle_gps_rtcm_data(const mavlink_message_t* msg)
{
    ...
    rt_bool_t should_clear_previous_fragments = false;

    if (rtcm_buffer->fragments_received) {
        const rt_bool_t sequence_nr_changed = rtcm_buffer->sequence != sequence;
        const rt_bool_t seen_this_fragment_index = rtcm_buffer->fragments_received & (1U << fragment);
