...
typedef struct {
    ...
    uint16_t period;
    uint32_t time_stamp;
    rt_bool_t (*msg_pack_cb)(mavlink_message_t* msg_t);
} mav_period_msg;
...
static void dump_immediate_msg(uint8_t chan)
{
    ...
    while (mav_handle.imm_mq[chan].head != mav_handle.imm_mq[chan].tail) {
        if (mavproxy_send_immediate_msg(chan, &mav_handle.imm_mq[chan].queue[mav_handle.imm_mq[chan].tail], true) == RT_EOK) {
            rt_enter_critical();
 ...
 * @return FMT Errors
 */
rt_err_t mavproxy_register_period_msg(uint8_t chan, uint8_t msgid, uint16_t msg_rate_hz,
                                       msg_pack_cb_t msg_pack_cb, rt_bool_t start)
{
    mav_period_msg msg;
    ...
    msg.time_stamp = rt_tick_get_millisecond() + mav_handle.period_mq[chan].size * MAVPROXY_INTERVAL;
...
rt_err_t mavproxy_send_immediate_msg(uint8_t chan, const mavlink_message_t* msg, rt_bool_t sync)
{
    ...
    if (sync) {
        ...
        size = mavproxy_dev_write(chan, mav_handle.tx_buffer[chan], len, 500);

        rt_sem_release(mav_handle.tx_lock[chan]);

        return size == len ? RT_EOK : RT_ERROR;
    }
    ...
    while (1) {
        ...
        if (res == RT_EOK) {
            ...
            if (mav_handle.devid[chan] != mav_handle.new_devid[chan]) {
                if (mavproxy_switch_dev(chan, mav_handle.new_devid[chan]) == RT_EOK) {
...
rt_err_t mavproxy_init(void)
{
    ...
    RT_ASSERT((mav_handle.tx_lock[0] != NULL) && (mav_handle.tx_lock[1] != NULL));
    ...
    RT_ASSERT((mav_handle.tx_buffer[0] != NULL) && (mav_handle.tx_buffer[1] != NULL));
    ...
    rt_timer_init(&mav_handle.timer[0], "mav_chan0", mavproxy_chan0_timer_update, RT_NULL, MAVPROXY_INTERVAL, RT_TIMER_FLAG_PERIODIC);
    RT_CHECK(rt_timer_start(&mav_handle.timer[0]));
    rt_timer_init(&mav_handle.timer[1], "mav_chan1", mavproxy_chan1_timer_update, RT_NULL, MAVPROXY_INTERVAL, RT_TIMER_FLAG_PERIODIC);
    RT_CHECK(rt_timer_start(&mav_handle.timer[1]));

    /* create mavproxy monitor to handle received mavlink msgs */
    RT_CHECK(mavproxy_monitor_create());
