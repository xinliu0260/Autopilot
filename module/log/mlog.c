...
#ifndef MLOG_BUFFER_SIZE
#define MLOG_BUFFER_SIZE (64*1024)
#endif

#ifndef MLOG_SECTOR_SIZE
#define MLOG_SECTOR_SIZE 512
#endif


#ifndef MLOG_MAX_SECTOR_TO_WRITE
#define MLOG_MAX_SECTOR_TO_WRITE 64
#endif


//#define WRITE_PAYLOAD(_payload, _len) write(mlog_handle.fid, _payload, _len);
#define WRITE_PAYLOAD(_payload, _len) buffer_write((const uint8_t*)_payload, _len);
...
rt_used const int __fmt_mlog_start rt_section("MlogTab.0");
rt_used const int __fmt_mlog_end rt_section("MlogTab.2");


rt_align(32) uint8_t __mlog_data_buffer[MLOG_BUFFER_SIZE];
static mlog_bus_t* __mlog_table;
static uint8_t __mlog_bus_num;
struct fmt_mlog mlog_handle;
static rt_list_t __start_cb_list_head = {
    .next = &__start_cb_list_head,
    .prev = &__start_cb_list_head,
};

static rt_list_t __stop_cb_list_head = {
    .next = &__stop_cb_list_head,
    .prev = &__stop_cb_list_head,
};

static rt_list_t __update_cb_list_head = {
    .next = &__update_cb_list_head,
    .prev = &__update_cb_list_head,
};
...
static void invoke_callback_func(mlog_cb_type type)
{
    ...
    if (type == MLOG_CB_START) {
        rt_list_for_each_entry(pos, &__start_cb_list_head, link)
        {
            /* invoke registered callback function */
            pos->func();
        }
    } else if (type == MLOG_CB_STOP) {
        rt_list_for_each_entry(pos, &__stop_cb_list_head, link)
        {
            /* invoke registered callback function */
            pos->func();
        }
    } else if (type == MLOG_CB_UPDATE) {
        rt_list_for_each_entry(pos, &__update_cb_list_head, link)
        {
            /* invoke registered callback function */
            pos->func();
        }
...
static rt_bool_t buffer_is_full(uint32_t len_to_write)
{
    uint32_t free_space_in_sector = MLOG_SECTOR_SIZE - mlog_handle.buffer.index;
...
rt_err_t mlog_register_callback(mlog_cb_type type, void (*cb_func)(void))
{
    ...
    if (type == MLOG_CB_START) {
        rt_list_insert_before(&__start_cb_list_head, &node->link);
    } else if (type == MLOG_CB_STOP) {
        rt_list_insert_before(&__stop_cb_list_head, &node->link);
    } else if (type == MLOG_CB_UPDATE) {
        rt_list_insert_before(&__update_cb_list_head, &node->link);
    }
...
int mlog_get_bus_id(const char* bus_name)
{
    ...
    mlog_bus_t* mlog_table = (mlog_bus_t*)((uint32_t)(&__fmt_mlog_start)+4);;
...
rt_err_t mlog_start(char* file_name)
{
    ...
    mlog_handle.header.timestamp = rt_tick_get_millisecond();
...
void mlog_async_output(void)
{
    ...
    while (head_p != tail_p) {
		...
		uint32_t addr = (uint32_t)&mlog_handle.buffer.data[tail_p * MLOG_SECTOR_SIZE];
    ...
    if (mlog_handle.log_status == MLOG_STATUS_STOPPING) {
        ...
        for (uint8_t i = 0; i < __mlog_bus_num; i++) {
            LOG_I( "%-20s id:%-3d record:%-8d lost:%-5d", __mlog_table[i].name, i, mlog_handle.stats[i].total_msg, mlog_handle.stats[i].lost_msg);
        }
    }
}
...
{
    __mlog_table = (mlog_bus_t*)((uint32_t)(&__fmt_mlog_start)+4);
