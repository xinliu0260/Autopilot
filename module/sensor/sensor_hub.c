...
static int echo_sensor_optflow(void* param)
{
    ...
    if (mcn_copy_from_hub((McnHub*)param, &optflow_report) != RT_EOK) {
        return -1;
    }
...
static int echo_sensor_rangefinder(void* param)
{
    ...
    if (mcn_copy_from_hub((McnHub*)param, &rf_report) != RT_EOK) {
        return -1;
    }
 ...
 */
rt_err_t advertise_sensor_imu(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_imu0_0), echo_sensor_imu));
        RT_TRY(mcn_advertise(MCN_HUB(sensor_imu0), echo_sensor_imu));
        break;
    case 1:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_imu1_0), echo_sensor_imu));
        RT_TRY(mcn_advertise(MCN_HUB(sensor_imu1), echo_sensor_imu));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_mag(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_mag0_0), echo_sensor_mag));
        RT_TRY(mcn_advertise(MCN_HUB(sensor_mag0), echo_sensor_mag));
        break;
    case 1:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_mag1_0), echo_sensor_mag));
        RT_TRY(mcn_advertise(MCN_HUB(sensor_mag1), echo_sensor_mag));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_baro(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_baro), echo_sensor_baro));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_airspeed(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_airspeed), echo_sensor_airspeed));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_gps(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_gps), echo_sensor_gps));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_optflow(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_optflow), echo_sensor_optflow));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t advertise_sensor_rangefinder(uint8_t id)
{
    switch (id) {
    case 0:
        RT_TRY(mcn_advertise(MCN_HUB(sensor_rangefinder), echo_sensor_rangefinder));
        break;
    default:
        return RT_EINVAL;
    }
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t register_sensor_imu(const char* gyr_dev_name, const char* acc_dev_name, uint8_t id)
{
    RT_ASSERT(id < MAX_IMU_DEV_NUM);
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t register_sensor_mag(const char* dev_name, uint8_t id)
{
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t register_sensor_barometer(const char* dev_name)
{
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t register_sensor_gps(const char* dev_name)
{
...
rt_err_t register_sensor_optflow(const char* dev_name)
{
    // TODO
    RT_TRY(mcn_advertise(MCN_HUB(sensor_optflow), NULL));
    return RT_EOK;
}

rt_err_t register_sensor_rangefinder(const char* dev_name)
{
    // TODO
    RT_TRY(mcn_advertise(MCN_HUB(sensor_rangefinder), NULL));
    return RT_EOK;
}
 ...
 * @return rt_err_t RT_EOK for success
 */
rt_err_t register_sensor_airspeed(const char* dev_name)
{
...
void sensor_collect(void)
{
     ...
     */
    imu_data_t imu_data;
    PERIOD_EXECUTE(imu, 1,
        {
            imu_data.timestamp_ms = rt_tick_get_millisecond();

            /* Collect imu0 data */
            if (imu_dev[0] != NULL) {
                if (sensor_gyr_measure(imu_dev[0], imu_data.gyr_B_radDs) == RT_EOK
                    && sensor_acc_measure(imu_dev[0], imu_data.acc_B_mDs2) == RT_EOK) {
                    /* do board rotation */
            ...
            /* Collect imu1 data */
            if (imu_dev[1] != NULL) {
                if (sensor_gyr_measure(imu_dev[1], imu_data.gyr_B_radDs) == RT_EOK
                    && sensor_acc_measure(imu_dev[1], imu_data.acc_B_mDs2) == RT_EOK) {
                    /* do board rotation */
                    rotation(board_rot, &imu_data.gyr_B_radDs[0], &imu_data.gyr_B_radDs[1], &imu_data.gyr_B_radDs[2]);
                    rotation(board_rot, &imu_data.acc_B_mDs2[0], &imu_data.acc_B_mDs2[1], &imu_data.acc_B_mDs2[2]);
                    /* publish scaled imu data without calibration and filtering */
     ...
     */
    mag_data_t mag_data;
    PERIOD_EXECUTE(mag, 1,
        {
            mag_data.timestamp_ms = rt_tick_get_millisecond();

            /* Collect mag0 data */
            if (mag_dev[0] != NULL) {
                if (sensor_mag_measure(mag_dev[0], mag_data.mag_B_gauss) == RT_EOK) {
                    /* do board rotation */
            ...
            /* Collect mag1 data */
            if (mag_dev[1] != NULL) {
                if (sensor_mag_measure(mag_dev[1], mag_data.mag_B_gauss) == RT_EOK) {
                    /* do board rotation */
                    rotation(board_rot, &mag_data.mag_B_gauss[0], &mag_data.mag_B_gauss[1], &mag_data.mag_B_gauss[2]);
                    /* publish scaled mag data without calibration and filtering */
    ...
    if (baro_dev != NULL) {
        if (sensor_baro_check_ready(baro_dev)) {
            if (sensor_baro_read(baro_dev, &baro_data) == RT_EOK) {
    ...
    PERIOD_EXECUTE(airspeed, 1, 
        {
            if (airspeed_dev != NULL) {
                if (sensor_airspeed_measure(airspeed_dev, &airspeed_data) == RT_EOK) {
                    /* publish barometer data */
