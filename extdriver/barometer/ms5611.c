...
#define EVENT_MS5611_UPDATE     (1<<0)
...
static void thread_entry(void* args)
{
    ...
    rt_uint32_t wait_set = EVENT_MS5611_UPDATE;

    while (1) {
        /* wait event occur */
        res = rt_event_recv(&event, wait_set, RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR, RT_WAITING_FOREVER, &recv_set);

        if (res == RT_EOK && (recv_set & EVENT_MS5611_UPDATE)) {
            ms5611_measure(RT_NULL);
        }
    }
}
...
{
    rt_event_send(&event, EVENT_MS5611_UPDATE);
}
...
rt_err_t drv_ms5611_init(const char* spi_device_name, const char* baro_device_name)
{
    ...
    RT_CHECK(rt_event_init(&event, "ms5611", RT_IPC_FLAG_FIFO));

    rt_thread_t tid = rt_thread_create("ms5611", 
                                    thread_entry, 
                                    RT_NULL, 
                                    2 * 1024, 
                                    7, 
    ...
    rt_timer_init(&timer, 
                        ...
                        timer_update, 
                        RT_NULL, 
                        CONV_TIME_INTERVAL[DEFAULT_OSR], 
    ...
    RT_TRY(dev_baro_register(&baro_device, baro_device_name, RT_DEVICE_FLAG_RDWR, RT_NULL));
