...
#define LOG_TAG "drv.ms4525"
#define LOG_LVL DBG_INFO
#include "ulog.h"

#define Airspeed_MS4525_Address 0X50
...
static rt_err_t ms4525_collect(uint8_t* val)
{
    ...
    struct rt_i2c_bus_device* i2c_device = (struct rt_i2c_bus_device*)i2c_dev;

    msgs.addr = Airspeed_MS4525_Address;
    ...
    while (1) {
        ...
        if (res == RT_EOK && (recv_set & EVENT_MS4525_UPDATE)) {
            if (ms4525_read(NULL, 0, report, 8) == 8) {
                ...
                airspeed_data.timestamp_ms = rt_tick_get_millisecond();
...
rt_err_t drv_ms4525_init(const char* i2c_device_name, const char* device_name)
{
    ...
    if (device_name != NULL) {
        RT_TRY(dev_airspeed_register(&airspeed_dev, device_name, RT_DEVICE_FLAG_RDWR, RT_NULL));
    } else {
        ...
        thread = rt_thread_create("ms4525", 
                                    thread_entry, 
                                    RT_NULL, 
                                    2 * 1024, 
                                    7, 
