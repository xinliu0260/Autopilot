...
static int ubx_rx_handle(void)
{
    ...
    case UBX_MSG_NAV_PVT: {
        ...
        gps_report.timestamp_time     = rt_tick_get_millisecond();
        gps_report.timestamp_velocity = rt_tick_get_millisecond();
        gps_report.timestamp_variance = rt_tick_get_millisecond();
        gps_report.timestamp_position = rt_tick_get_millisecond();
    ...
    case UBX_MSG_NAV_SVINFO: {
        ...
        ubx_decoder.satellite_info.timestamp = autopilot_system_time_us();
...
static int wait_for_ack(const uint16_t msg, const uint32_t timeout)
{
    ...
    uint32_t time_started = rt_tick_get_millisecond();

    while ((ubx_decoder.ack_state == UBX_ACK_WAITING) && (rt_tick_get_millisecond() < time_started + timeout)) {
        ;
    }
...
static rt_err_t probe(uint32_t* gps_baudrate)
{
    ...
    for (i = 0; i < sizeof(baudrates) / sizeof(baudrates[0]); i++) {
        ...
        rt_memset(&ubx_decoder.buf.payload_tx_cfg_prt, 0, sizeof(ubx_decoder.buf.payload_tx_cfg_prt));
...
static rt_err_t configure_by_ubx(uint32_t baudrate)
{
    ...
    rt_memset(&ubx_decoder.buf.payload_tx_cfg_rate, 0, sizeof(ubx_decoder.buf.payload_tx_cfg_rate));
    ...
    rt_memset(&ubx_decoder.buf.payload_tx_cfg_nav5, 0, sizeof(ubx_decoder.buf.payload_tx_cfg_nav5));
...
static void gps_probe_entry(void* parameter)
{
    ...
    for (i = 0; i < CONFIGURE_RETRY_MAX; i++) {
        if (probe(&baudrate) == RT_EOK) {
            if (configure_by_ubx(baudrate) == RT_EOK) {
                ...
                dev_gps_register(&gps_device, "gps", RT_DEVICE_FLAG_RDWR, RT_NULL);
    ...
    if (i >= CONFIGURE_RETRY_MAX) {
        LOG_W("GPS configuration fail! Please check if GPS module has connected.");
    }
...
rt_err_t gps_ubx_init(const char* serial_device_name, const char* gps_device_name)
{
    ...
    rt_memset(str_buffer, 0, 21);
    rt_strncpy(str_buffer, gps_device_name, 20);
    ...
    /* init ublox decoder */
    RT_CHECK(init_ubx_decoder(&ubx_decoder, serial_device, ubx_rx_handle));
