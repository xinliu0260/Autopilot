...
static rt_err_t icm20948_who_am_i()
{
    ...
    if (icm20948_id == ICM20948_ID) {
        LOG_I("ICM20948 right device id:0x%X\n", icm20948_id);
        return RT_EOK;

    } else {
        LOG_I("ICM20948 wrong device id:0x%X\n", icm20948_id);
        return RT_ERROR;
    }
}
...
static rt_err_t ak09916_who_am_i()
{
    ...
    if (ak09916_id == AK09916_ID) {
        LOG_I("AK09916 right device id:0x%X\n", ak09916_id);
        return RT_EOK;
    } else {
        LOG_I("AK09916 wrong device id:0x%X\n", ak09916_id);
        return RT_ERROR;
    }
}
...
static void probe_ak09916(int16_t* probe_mag)
{
    ...
    case 0:
        ...
        t_start = autopilot_system_time_us();
        step    = 1;
        break;

    case 1:
        if ((autopilot_system_time_us() - t_start) > 1000) {
...
rt_err_t drv_icm20948_init(const char* spi_device_name, const char* gyro_device_name, const char* accel_device_name, const char* mag_device_name)
{
    ...
    rt_timer_init(&timer, "ak09916", timer_update, RT_NULL, 2, RT_TIMER_FLAG_PERIODIC | RT_TIMER_FLAG_HARD_TIMER);
    if (rt_timer_start(&timer) != RT_EOK) {
        return RT_ERROR;
    }

    /* register gyro hal device */
    RT_TRY(dev_gyro_register(&gyro_dev, gyro_device_name, RT_DEVICE_FLAG_RDWR, RT_NULL));
    /* register accel hal device */
    RT_TRY(dev_accel_register(&accel_dev, accel_device_name, RT_DEVICE_FLAG_RDWR, RT_NULL));
