...
static rt_err_t low_level_init(void)
{
    ...
    if (dev_id != DEVICE_ID) {
        LOG_I("ICM42688P wrong device id:0x%x\n", dev_id);
        return RT_ENOSYS;
    }
...
rt_err_t drv_icm42688_init(const char* spi_dev_name, const char* gyro_dev_name, const char* accel_dev_name, uint32_t dev_id)
{
    ...
    RT_TRY(dev_gyro_register(&gyro_dev, gyro_dev_name, RT_DEVICE_FLAG_RDWR, (void*)dev_id));
    /* register accel hal device */
    RT_TRY(dev_accel_register(&accel_dev, accel_dev_name, RT_DEVICE_FLAG_RDWR, (void*)dev_id));
