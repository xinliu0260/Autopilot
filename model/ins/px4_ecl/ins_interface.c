...
{
    // RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_predict_us), &px4_ecl_params.ekf2_predict_us));
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_gyr_noise), &px4_ecl_params.ekf2_gyr_noise)); ///< IMU angular rate noise used for covariance prediction (rad/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_acc_noise), &px4_ecl_params.ekf2_acc_noise)); ///< IMU acceleration noise use for covariance prediction (m/sec**2)
    // process noise
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_gyr_b_noise), &px4_ecl_params.ekf2_gyr_b_noise)); ///< process noise for IMU rate gyro bias prediction (rad/sec**2)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_acc_b_noise), &px4_ecl_params.ekf2_acc_b_noise)); ///< process noise for IMU accelerometer bias prediction (m/sec**3)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_e_noise), &px4_ecl_params.ekf2_mag_e_noise)); ///< process noise for earth magnetic field prediction (Gauss/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_b_noise), &px4_ecl_params.ekf2_mag_b_noise)); ///< process noise for body magnetic field prediction (Gauss/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_wind_noise), &px4_ecl_params.ekf2_wind_noise));   ///< process noise for wind velocity prediction (m/sec**2)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_noaid_noise), &px4_ecl_params.ekf2_noaid_noise)); ///< observation noise for non-aiding position fusion (m)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_baro_noise), &px4_ecl_params.ekf2_baro_noise));   ///< observation noise for barometric height fusion (m)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_gnd_eff_dz), &px4_ecl_params.ekf2_gnd_eff_dz));   ///< barometric deadzone range for negative innovations (m)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_gnd_max_hgt), &px4_ecl_params.ekf2_gnd_max_hgt)); ///< maximum height above the ground level for expected negative baro innovations (m)
    // control of magnetometer fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_head_noise), &px4_ecl_params.ekf2_head_noise)); ///< measurement noise used for simple heading fusion (rad)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_noise), &px4_ecl_params.ekf2_mag_noise));   ///< measurement noise used for 3-axis magnetoemeter fusion (Gauss)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_eas_noise), &px4_ecl_params.ekf2_eas_noise));   ///< measurement noise used for airspeed fusion (m/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_beta_noise), &px4_ecl_params.ekf2_beta_noise)); ///< synthetic sideslip noise (rad)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_decl), &px4_ecl_params.ekf2_mag_decl));     ///< magnetic declination (degrees)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_decl_type), &px4_ecl_params.ekf2_decl_type));   ///< bitmask used to control the handling of declination data
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_type), &px4_ecl_params.ekf2_mag_type));     ///< integer used to specify the type of magnetometer fusion used
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_gps_check), &px4_ecl_params.ekf2_gps_check));   ///< bitmask used to control which GPS quality checks are used
    // measurement source control
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_aid_mask), &px4_ecl_params.ekf2_aid_mask));   ///< bitmasked integer that selects which of the GPS and optical flow aiding sources will be used
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_hgt_mode), &px4_ecl_params.ekf2_hgt_mode));   ///< selects the primary source for height data
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_terr_mask), &px4_ecl_params.ekf2_terr_mask)); ///< bitmasked integer that selects which of range finder and optical flow aiding sources will be used for terrain estimation
    // range finder fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_rng_noise), &px4_ecl_params.ekf2_rng_noise)); ///< observation noise for range finder measurements (m)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_rng_aid), &px4_ecl_params.ekf2_rng_aid));     ///< enables use of a range finder even if primary height source is not range finder
    // vision estimate fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_ev_noise_md), &px4_ecl_params.ekf2_ev_noise_md)); ///< determine source of vision observation noise
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_evp_noise), &px4_ecl_params.ekf2_evp_noise));     ///< default position observation noise for exernal vision measurements (m)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_evv_noise), &px4_ecl_params.ekf2_evv_noise));     ///< default velocity observation noise for exernal vision measurements (m/s)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_eva_noise), &px4_ecl_params.ekf2_eva_noise));     ///< default angular observation noise for exernal vision measurements (rad)
    // optical flow fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_of_n_min), &px4_ecl_params.ekf2_of_n_min)); ///< best quality observation noise for optical flow LOS rate measurements (rad/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_of_n_max), &px4_ecl_params.ekf2_of_n_max)); ///< worst quality observation noise for optical flow LOS rate measurements (rad/sec)
    // control of airspeed and sideslip fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_arsp_thr), &px4_ecl_params.ekf2_arsp_thr));   ///< A value of zero will disabled airspeed fusion. Any positive value sets the minimum airspeed which will be used (m/sec)
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_fuse_beta), &px4_ecl_params.ekf2_fuse_beta)); ///< Controls synthetic sideslip fusion, 0 disables, 1 enables
    // Multi-rotor drag specific force fusion
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_drag_noise), &px4_ecl_params.ekf2_drag_noise));           ///< observation noise variance for drag specific force measurements (m/sec**2)**2
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_mag_check), &px4_ecl_params.ekf2_mag_check));             ///< Mag field strength check
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_synthetic_mag_z), &px4_ecl_params.ekf2_synthetic_mag_z)); ///< Enables the use of a synthetic value for the Z axis of the magnetometer calculated from the 3D magnetic field vector at the location of the drone.
    RT_CHECK(param_link_variable(PARAM_GET(INS, ekf2_req_gps_h), &px4_ecl_params.ekf2_req_gps_h));             ///< Required GPS health time
}
...
void ins_interface_init(void)
{
    ...
    RT_ASSERT(IMU_ID >= 0);
    RT_ASSERT(MAG_ID >= 0);
    RT_ASSERT(Barometer_ID >= 0);
    RT_ASSERT(GPS_ID >= 0);
    RT_ASSERT(Rangefinder_ID >= 0);
    RT_ASSERT(OpticalFlow_ID >= 0);
