...
{
    RT_CHECK(param_link_variable(PARAM_GET(PLANT, LAT_0), &PLANT_PARAM.LAT_0));
    RT_CHECK(param_link_variable(PARAM_GET(PLANT, LON_0), &PLANT_PARAM.LON_0));
    RT_CHECK(param_link_variable(PARAM_GET(PLANT, ALT_0), &PLANT_PARAM.ALT_0));
}
uint64_t time_start;
uint64_t time_use;
...
void plant_interface_step(uint32_t timestamp)
{
    ...
    if (mcn_poll(environment_info_nod)) {
        ...
        Plant_U.Environment_Info.timestamp = rt_tick_get_millisecond();
    ...
    if (mcn_poll(states_init_nod)) {
        ...
        Plant_U.States_Init.timestamp = rt_tick_get_millisecond();

        states_init_updated = 1;
    }

    /* run plant model */
	uint64_t time_start = autopilot_system_time_us();
    Plant_step();
	time_use = autopilot_system_time_us()-time_start;
