...
#include "drv_usart_v2_private.h"
#include "rttypes.h"
                                       ...
                                       rt_uint32_t           tx_flag)
{
    ...
    if (uart->uart_dma_flag & RT_DEVICE_FLAG_DMA_TX)
    {
        ...
        SCB_CleanDCache_by_Addr((void*)buf, size);
		#endif
...
static void dma_recv_isr(struct rt_serial_device *serial, rt_uint8_t isr_flag)
{
    ...
    if (recv_len)
    {
        ...
        SCB_SafeInvalidateDCache_by_Addr((uint32_t *)rx_fifo->buffer, serial->config.rx_bufsz);
#endif
...
int rt_hw_usart_init(void)
{
    ...
    // uart1 
    {
        static struct dma_config dma_rx = UART1_DMA_RX_CONFIG;
        uart_config[UART1_INDEX].dma_rx = &dma_rx;
        uart_obj[UART1_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_RX ;

        static struct dma_config dma_tx = UART1_DMA_TX_CONFIG;
        uart_config[UART1_INDEX].dma_tx = &dma_tx;
        uart_obj[UART1_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_TX ;

        uart_obj[UART1_INDEX].config = &uart_config[UART1_INDEX];
       
        struct serial_configure serial_config = UART1_SERIAL_USER_CONFIGS;
        uart_obj[UART1_INDEX].serial.config = serial_config;
        uart_obj[UART1_INDEX].serial.ops = &stm32_uart_ops;

        result = rt_hw_serial_register(&uart_obj[UART1_INDEX].serial,
                                        uart_obj[UART1_INDEX].config->name, 
                                        RT_DEVICE_FLAG_RDWR, 
                                        RT_NULL);
    ...
    //uart2
    {
        static struct dma_config dma_rx = UART2_DMA_RX_CONFIG;
        uart_config[UART2_INDEX].dma_rx = &dma_rx;
        uart_obj[UART2_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_RX ;

        static struct dma_config dma_tx = UART2_DMA_TX_CONFIG;
        uart_config[UART2_INDEX].dma_tx = &dma_tx;
        uart_obj[UART2_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_TX ;

        uart_obj[UART2_INDEX].config = &uart_config[UART2_INDEX];

        struct serial_configure serial_config = UART2_SERIAL_USER_CONFIGS;
        uart_obj[UART2_INDEX].serial.config = serial_config;
        uart_obj[UART2_INDEX].serial.ops = &stm32_uart_ops;

        result = rt_hw_serial_register(&uart_obj[UART2_INDEX].serial,
                                        uart_obj[UART2_INDEX].config->name, 
                                        RT_DEVICE_FLAG_RDWR, 
                                        RT_NULL);
        RT_ASSERT(result == RT_EOK);
    }
    //uart5
    {
        static struct dma_config dma_rx = UART5_DMA_RX_CONFIG;
        uart_config[UART5_INDEX].dma_rx = &dma_rx;
        uart_obj[UART5_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_RX ;

        // static struct dma_config dma_tx = UART5_DMA_TX_CONFIG;
        // uart_config[UART5_INDEX].dma_tx = &dma_tx;
        // uart_obj[UART5_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_TX ;

        uart_obj[UART5_INDEX].config = &uart_config[UART5_INDEX];

        struct serial_configure serial_config = UART5_SERIAL_USER_CONFIGS;
        uart_obj[UART5_INDEX].serial.config = serial_config;
        uart_obj[UART5_INDEX].serial.ops = &stm32_uart_ops;

        result = rt_hw_serial_register(&uart_obj[UART5_INDEX].serial,
                                        uart_obj[UART5_INDEX].config->name, 
    ...
    //uart6
    {
        static struct dma_config dma_rx = UART6_DMA_RX_CONFIG;
        uart_config[UART6_INDEX].dma_rx = &dma_rx;
        uart_obj[UART6_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_RX ;

        static struct dma_config dma_tx = UART6_DMA_TX_CONFIG;
        uart_config[UART6_INDEX].dma_tx = &dma_tx;
        uart_obj[UART6_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_TX ;

        uart_obj[UART6_INDEX].config = &uart_config[UART6_INDEX];

        struct serial_configure serial_config = UART6_SERIAL_USER_CONFIGS;
        uart_obj[UART6_INDEX].serial.config = serial_config;
        uart_obj[UART6_INDEX].serial.ops = &stm32_uart_ops;

        result = rt_hw_serial_register(&uart_obj[UART6_INDEX].serial,
                                        uart_obj[UART6_INDEX].config->name, 
                                        RT_DEVICE_FLAG_RDWR, 
                                        RT_NULL);
        RT_ASSERT(result == RT_EOK);
    }
    //uart8
    {
        static struct dma_config dma_rx = UART8_DMA_RX_CONFIG;
        uart_config[UART8_INDEX].dma_rx = &dma_rx;
        uart_obj[UART8_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_RX ;

        static struct dma_config dma_tx = UART8_DMA_TX_CONFIG;
        uart_config[UART8_INDEX].dma_tx = &dma_tx;
        uart_obj[UART8_INDEX].uart_dma_flag |= RT_DEVICE_FLAG_DMA_TX ;

        uart_obj[UART8_INDEX].config = &uart_config[UART8_INDEX];

        struct serial_configure serial_config = UART8_SERIAL_USER_CONFIGS;
        uart_obj[UART8_INDEX].serial.config = serial_config;
        uart_obj[UART8_INDEX].serial.ops = &stm32_uart_ops;

        result = rt_hw_serial_register(&uart_obj[UART8_INDEX].serial,
                                        uart_obj[UART8_INDEX].config->name, 
                                        RT_DEVICE_FLAG_RDWR, 
                                        RT_NULL);
        RT_ASSERT(result == RT_EOK);
    }
